...
この点について、掲載書籍の冒頭でも紹介されているように、MAツールはPDCAの「D」部分を自動化するもので、その他工程はマーケター自身が、シナリオを設計しなければならない。
...
ちなみに以下の書籍では、AIDMAモデルで丁寧に説明されており、AIDMA（認知 / 興味 / 欲求 / 記憶 / 行動）の **IDM部分のみ** を自動化するツールであるとしている。
